...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:tei="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="xd tei" version="1.0">
  ...
  <xsl:template match="/">
    <xsl:comment> EM-TEI-Assing-note-numbers </xsl:comment>
    <xsl:apply-templates />
  </xsl:template>
  <xsl:template match="tei:note   [not(parent::tei:fw)]   [not(parent::tei:del)]   [not(ancestor::*/self::tei:note[not(@place='margin')])]   [not(@place='margin') and not(@type='print')]   ">
    <xsl:copy>
      <xsl:copy-of select="@*" />
      ...
      <xsl:attribute name="xml:id">
        ...
        <xsl:number count="tei:note" format="1" from="tei:text" level="any" />
      </xsl:attribute>
      <xsl:apply-templates />
    </xsl:copy>
  </xsl:template>
...
-->
      <xsl:otherwise>
        ...
        <xsl:number count="tei:note      [not(tei:choice[tei:sic])]      [not(ancestor::*/self::tei:note[not(@place='margin')])]      [not(@place='margin') and not(@type='print')]       |       tei:note      [not(@place='margin') and not(@type='print')]       [not(tei:choice)]      [not(ancestor::*/self::tei:note[not(@place='margin')])]      " format="1" from="tei:front | tei:body" level="any" />
